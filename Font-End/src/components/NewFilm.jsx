...
import { Image as ImageIcon, Video, Play, X, Plus, Film, Star, Calendar, File, Upload, Folder, Crown } from 'lucide-react';

// ─── FileUploadBox ────────────────────────────────────────────────────────────
const FileUploadBox = ({ fileType, file, preview, error, accept, title, icon: Icon, onFileChange, onRemove, inputRef }) => (
  <div
    className={`relative border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer
      ${error ? 'border-red-500 bg-red-500/5' : 'border-gray-700 hover:border-red-500 hover:bg-red-500/5'}`}
...
// ─── AddMovieComponent ────────────────────────────────────────────────────────
const AddMovieComponent = ({ onClose, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.videoFile)          e.video       = 'Vui lòng chọn file video chính';
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/movies', { method: 'POST', body: form });
    ...
    <div className="text-white space-y-6">
      ...
      <div className="bg-gray-900/50 rounded-2xl p-6 border border-gray-800">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          <Film className="w-5 h-5 text-red-500" /> Thông Tin Cơ Bản
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Tên phim */}
          <div>
            <label className="flex items-center gap-2 text-sm font-medium mb-2">
              <Film className="w-4 h-4 text-red-500" /> Tên Phim *
            </label>
            <input type="text" name="title" value={formData.title} onChange={handleInputChange}
              ...
              className={`w-full px-4 py-3 bg-gray-900 border rounded-xl focus:outline-none focus:ring-2 transition-all
                ${errors.title ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:border-red-500 focus:ring-red-500/30'}`} />
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium mb-2">
              <Star className="w-4 h-4 text-red-500" /> Thể Loại Chính *
            </label>
            <input type="text" name="genre" value={formData.genre} onChange={handleInputChange}
              placeholder="Ví dụ: Hành động, Kinh dị..."
              className={`w-full px-4 py-3 bg-gray-900 border rounded-xl focus:outline-none focus:ring-2 transition-all
                ${errors.genre ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:border-red-500 focus:ring-red-500/30'}`} />
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium mb-2">
              <Calendar className="w-4 h-4 text-red-500" /> Ngày Phát Hành *
            </label>
            <input type="date" name="releaseDate" value={formData.releaseDate} onChange={handleInputChange}
              className={`w-full px-4 py-3 bg-gray-900 border rounded-xl focus:outline-none focus:ring-2 transition-all
                ${errors.releaseDate ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:border-red-500 focus:ring-red-500/30'}`} />
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium mb-2">
              <File className="w-4 h-4 text-red-500" /> Mô Tả Phim *
            </label>
            <textarea name="description" value={formData.description} onChange={handleInputChange}
              ...
              className={`w-full px-4 py-3 bg-gray-900 border rounded-xl focus:outline-none focus:ring-2 transition-all resize-none
                ${errors.description ? 'border-red-500 focus:ring-red-500/30' : 'border-gray-700 focus:border-red-500 focus:ring-red-500/30'}`} />
      ...
      <div className="bg-gray-900/50 rounded-2xl p-6 border border-gray-800">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          <Upload className="w-5 h-5 text-red-500" /> Tải Lên File Media
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div>
            ...
            <FileUploadBox fileType="poster" file={formData.posterFile} preview={formData.posterPreview}
              error={errors.poster} accept="image/*" title="Chọn Ảnh Bìa" icon={ImageIcon}
          ...
          <div>
            ...
            <FileUploadBox fileType="video" file={formData.videoFile} preview={formData.videoPreview}
              error={errors.video} accept="video/*" title="Chọn Video Chính" icon={Video}
          ...
          <div>
            ...
            <FileUploadBox fileType="trailer" file={formData.trailerFile} preview={formData.trailerPreview}
              error={errors.trailer} accept="video/*" title="Chọn Trailer" icon={Play}
      ...
      <div className="bg-gray-900/50 rounded-2xl p-6 border border-gray-800">
        <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
          <Star className="w-5 h-5 text-red-500" /> Thể Loại Phim *
        </h2>
        ...
        {formData.categories.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {formData.categories.map(cat => (
              <span key={cat} className="inline-flex items-center gap-1 px-3 py-1 bg-red-600/20 text-red-300 rounded-full text-sm">
                {cat}
      ...
      <div className="flex gap-4 pt-2">
        ...
        <button type="button" onClick={handleSubmit} disabled={isSubmitting}
          className="flex-1 px-8 py-4 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800
          ...
          {isSubmitting
            ...
            : <><Folder className="w-5 h-5" /> Lưu Phim</>}
        </button>
      </div>
    </div>
  );
